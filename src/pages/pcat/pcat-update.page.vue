...
import { UPDATE_FIELDS, COMMONS } from 'src/data/pcat';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const url = computed(() => `${COMMONS.BASE_API_URL}/${id.value}`);

    const {
      result: singleResult,
      getSingleEntity
    } = useSingleEntity(COMMONS.ENTITY_NAME);
    const initial = computed(() => singleResult.value.type == 'result' ? singleResult.value.data : {});

    const {
      result: updateResult,
      updateEntity
    } = useUpdateEntity(COMMONS.ENTITY_NAME);
