...
import { CREATE_FIELDS, COMMONS } from 'src/data/pcat';
...
export default defineComponent({
  ...
  setup() {
    const initial = { title: '' };
    const { createEntity, result: createResult } = useCreateEntity(COMMONS.ENTITY_NAME);
    const loading = computed(() => createResult.value.type == 'loading');
    const $router = useRouter();
    const onSubmit = (payload: any) => {
      createEntity(COMMONS.BASE_API_URL, payload)
        .then(() => {
          $router.back();
        });
    };
